...
function GetFiles(Path) {
    ...
    $.post('../getDir', data, function (rdata) {
        ...
        for (var i = 0; i < rdata.DIR.length; i++) {
            ...
            if (getCookie("rank") == "a") {
                ...
                Body += "<tr class='folderBoxTr' data-path='" + rdata.PATH + "/" + fmp[0] + "' filetype='dir'>\
						<td width='3%'><input type='checkbox' name='id' value='" + fmp[0] + "'></td>\
						<td width='18%' class='column-name'><span class='cursor' onclick=\"GetFiles('" + rdata.PATH + "/" + fmp[0] + "')\"><span class='ico ico-folder'></span><a class='text' title='" + fmp[0] + "'>" + cnametext + "</a></span></td>\
						<td width='7%'>" + ToSize(fmp[1]) + "</td>\
						<td width='15%'>" + getLocalTime(fmp[2]) + "</td>\
						<td width='10%'>" + fmp[3] + "</td>\
						<td width='17%' class='column-name'><span class='text' style='200px;' title='"+fmp[4]+"' >" + fmp[4] + "</span></td>\
						<td width='28%' class='editmenu'><span>\
        ...
        for (var i = 0; i < rdata.FILES.length; i++) {
            ...
            if (getCookie("rank") == "a") {
                Body += "<tr class='folderBoxTr' data-path='" + rdata.PATH + "/" + fmp[0] + "' filetype='" + fmp[0] + "'><td width='3%'><input type='checkbox' name='id' value='" + fmp[0] + "'></td>\
						<td width='18%' class='column-name'><span class='ico ico-" + (GetExtName(fmp[0])) + "'></span><a class='text' title='" + fmp[0] + "'>" + cnametext + "</a></td>\
						<td width='7%'>" + (ToSize(fmp[1])) + "</td>\
						<td width='15%'>" + getLocalTime(fmp[2]) + "</td>\
						<td width='10%'>" + fmp[3] + "</td>\
						<td width='17%' class='column-name'><span class='text' style='200px;' title='"+fmp[4]+"' >" + fmp[4] + "</span></td>\
						<td width='28%'class='editmenu'>\
        ...
        var dirInfo = '(共' + rdata.DIR.length + '个目录与' + rdata.FILES.length + '个文件,大小:<font id="pathSize">' + (ToSize(rdata.totalSize)) + '<a class="btlink ml5" onClick="GetPathSize()">获取</a></font>)';
            ...
            var tablehtml = '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-hover">\
							<thead>\
								<tr>\
									...
									<th width="10%">权限</th>\
									<th width="17%">所有者</th>\
									<th width="28%" style="text-align: right;" width="400">操作</th>\
								</tr>\
							</thead>\
...
function GetExtName(fileName) {
    ...
    var exts = ['sh', 'cnf', 'pl', 'so', 'passwd', 'cshrc', 'deny', 'cache', 'init', 'po', 'ext2', 'ext3', 'ext4', 'i686', 'img', 'gz', 'efi', 'old', 'pid', 'lock', 'frm', 'opt', 'err', 'MYI', 'MYD', 'CSM', 'ini', 'eps'];
    var extLastName = extArr[extArr.length - 1];
    console.log(extLastName);
    for (var i = 0; i < exts.length; i++) {
...
function GetDisk() {
    ...
    $.ajax({
        ...
        success: function (rdata) {
            LBody += "<span onclick=\"GetFiles('" + rdata.path + "')\"><span class='glyphicon glyphicon-hdd'></span>&nbsp;" + (rdata.path == '/' ? '根目录' : rdata.path) + "(" + rdata.volume[1] + ")</span>";
            $("#comlist").html(LBody);
        },
        error: function (err) {
            layer.msg("网络错误");
...
function CreateFile(type, path) {
    path = $("#DirPathPlace input").val().replace('//', '/');
    if (type == 1) {
        ...
        $.post('../CreateFile', 'path=' + path + '/' + fileName, function (rdata) {
            layer.closeAll();
            layer.msg(rdata.msg, {
...
function CreateDir(type, path) {
    if (type == 1) {
        ...
        $.post('../CreateDir', 'path=' + path + '/' + dirName, function (rdata) {
            layer.closeAll();
            layer.msg(rdata.msg, {
                icon: rdata.status ? 1 : 2
            });
...
function DeleteFile(fileName) {
    layer.confirm("您真的要删除[" + fileName + "]吗?", {title: '删除文件', closeBtn: 2, icon: 3}, function () {
        ...
        $.post('../DeleteFile', 'path=' + fileName, function (rdata) {
            layer.closeAll();
            layer.msg(rdata.msg, {
                icon: rdata.status ? 1 : 2
            });
...
function DeleteDir(dirName) {
    layer.confirm("您真的要删除[" + dirName + "]吗?", {title: '删除目录', closeBtn: 2, icon: 3}, function () {
        layer.msg('正在执行,请稍候...', {icon: 16, time: 0, shade: [0.3, '#000']});
        $.post('../DeleteDir', 'path=' + dirName, function (rdata) {
            layer.closeAll();
            layer.msg(rdata.msg, {
                icon: rdata.status ? 1 : 2
            });
...
function AllDeleteFileSub(data, path) {
    layer.confirm('您真的要删除这些文件吗?', {title: '批量删除文件', closeBtn: 2, icon: 3}, function () {
        layer.msg('正在执行,请稍候...', {icon: 16, time: 0, shade: [0.3, '#000']});
        $.post('../SetBatchData', data, function (rdata) {
            layer.closeAll();
            GetFiles(path);
            layer.msg(rdata.msg, {icon: 1});
        });
...
function DownloadFile(action) {
    ...
    if (action == 1) {
        ...
        $.post('../DownloadFile', 'path=' + fpath + '&url=' + fUrl + '&filename=' + fname, function (rdata) {
            layer.closeAll();
            GetFiles(fpath);
            GetTaskCount();
...
function PasteFile(fileName) {
    ...
    if (copyName != 'null' && copyName != undefined) {
        ...
        $.post('../CopyFile', 'sfile=' + copyName + '&dfile=' + path + '/' + fileName, function (rdata) {
            layer.closeAll();
            layer.msg(rdata.msg, {
                icon: rdata.status ? 1 : 2
            });
    ...
    if (cutName != 'null' && cutName != undefined) {
        ...
        $.post('../MoveFile', 'sfile=' + cutName + '&dfile=' + path + '/' + fileName, function (rdata) {
            layer.closeAll();
            layer.msg(rdata.msg, {
                icon: rdata.status ? 1 : 2
            });
...
function GetImage(fileName) {
    var imgUrl = '/download?path=' + fileName;
    layer.open({
        type: 1,
        closeBtn: 2,
        title: false,
...
function GetFileBytes(fileName, fileSize) {
    window.open('/download?path=' + fileName);
}
